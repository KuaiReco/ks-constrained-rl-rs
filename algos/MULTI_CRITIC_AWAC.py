...
from algos.MULTI_CRITIC_A3C import MULTI_CRITIC_A3C
...
class MULTI_CRITIC_AWAC(object):
	def __init__(self, args):
		...
		self.reward_dim = 8
		self.weight = [args.reward_service, args.reward_business,args.reward_cleanliness, args.reward_check_in, args.reward_value, args.reward_rooms, args.reward_location, args.reward_overall]


		if len(args.model_path) > 0: 
			self.load(args.model_path)

		self.constrained_policy = MULTI_CRITIC_A3C(args)
		self.constrained_policy.load(args.constrained_policy_model_path)
	...
	def train(self, replay_buffer, args, batch_size=64):
		...
		for i in range(self.reward_dim):
			...
			target_Q[i] = reward[:, i].reshape([-1, 1]) + not_done * self.discount * target_Q[i]
		...
		critic_loss = 0 
		for i in range(self.reward_dim):
			critic_loss += F.mse_loss(current_Q[i], target_Q[i].detach())
		...
		# Compute awac weights
		constrain_policy_dist = self.constrained_policy.actor(state)
		constrain_prob = torch.exp(constrain_policy_dist.log_prob(action))
		awac_weights = constrain_prob * F.softmax(adv * args.kappa, dim=0)
		...
		# Compute actor loss
		# policy loss+ bc loss
		actor_loss = -torch.mean(awac_weights.detach() * log_p + args.bc_loss_coeff * log_p)
